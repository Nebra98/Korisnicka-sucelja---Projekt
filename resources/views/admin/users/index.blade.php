                                ...
                                <tr>
                                    ...
                                    <td>
                                        @can('edit-users')
                                            ...
                                            @if($aa == '["user"]')
                                                <a href="{{ route('information.show', $user) }}"> <button type="button" style="margin-left: 5px" class="btn btn-info float-left">info</button></a>
                                            @endif
                                        @endcan
                                        ...
                                        <form action="{{ route('admin.users.destroy', $user) }}" method="POST" class="float-left">
                                            @csrf
                                            {{ method_field('DELETE') }}
                                            <button type="submit" style="margin-left: 5px" onclick="return confirm('Jeste li sigurni da želite izbrisati  korisnika {{$user->name}}?')" class="btn btn-danger">Izbriši</button>
